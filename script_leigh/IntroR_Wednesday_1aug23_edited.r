...
#Instructors: Leigh Bailey Jasmeen Kaur
...
getwd()
setwd("C:/Users/Melissa Bailey/Documents/R workshop/Introduction to R P2/")
...
# you type, and if you may ever need to send your code to others you'll 
# want to maintain consistency

# As a refresher, these are some ways you can select and remove 
...
install.packages("tidyverse") # Accept all to install
library(tidyverse) 
...
vireos <- read.csv("~/Desktop/CODE2023/introRp2/bcvi_movement.csv", header = TRUE)
view(vireos)
names(vireos)

plot(dist_moved_meters ~ mass, data = frogs)
                               #ANSWER: wrong data frame in the data field
cor.test(vireos$dist_moved_meters, frogs$mass, method="pearson")

plot(dist_moved_meters  ~ wing_length, data=vireos)
corr.test(vireos$dist_moved_meters, vireos$wing_length, method="pearson")  #ANSWER the function should be cor.test not corr.test

plot(dist_moved_meters ~ cap_ColorLevel, data=vireos)                              #ANSWER: the variable cap_ColorLevel should be cap_colorLevel, R is case sensitive
cor.test(vireos&dist_moved_meters, vireos&cap_colorLevel, method="pearson")       #ANSWER: The ampersand symbols should be replaced with the $ symbol

plot(dist_moved_meters ~ Julian_date, data=hoppers)
cor.test(vireos$dist_moved_meters, vireos$Julian_date, method="pearson")        #ANSWER: the variable Julian_date should be juliandate
